...
func (p *Parser) expectError(expectedKind lexer.TOKEN_KIND, err any) lexer.Token {
	...
	if kind != expectedKind {
		if err == nil {
			p.MakeError(p.currentToken().StartPos.Line, p.FilePath, token, fmt.Sprintf("Unexpected token '%s'", token.Value)).AddHint(fmt.Sprintf("How about trying '%s' instead?", expectedKind), TEXT_HINT).Display()
		} else {
